...
// === SISTEM PRODUK DINAMIS (AUTO UPDATE) ===
document.addEventListener('DOMContentLoaded', () => {
    const container = document.getElementById('product-container');
    
    // 1. Ambil data dari file products.json
    if (container) {
        fetch('products.json')
            ...
            .then(products => {
                ...
                // 2. Loop setiap produk dan buat HTML-nya
                products.forEach((produk, index) => {
                    // Tentukan posisi gambar (selang-seling kiri/kanan)
                    const isEven = index % 2 === 0;
                    const orderClassImg = isEven ? 'lg:order-1' : 'lg:order-2';
                    const orderClassText = isEven ? 'lg:order-2' : 'lg:order-1';
                    
                    // Template HTML (Sama persis dengan desain Anda)
                    const html = `
                    <div class="flex flex-col lg:flex-row items-center gap-8 md:gap-16 fade-in-section">
                        <div class="lg:w-1/2 relative ${orderClassImg}">
                            <img src="${produk.gambar}" alt="${produk.nama}" class="w-full max-w-md mx-auto transform transition duration-500 hover:scale-105 rounded-lg">
                        </div>
                        <div class="lg:w-1/2 ${orderClassText}">
                            <div class="bg-white p-8 rounded-2xl shadow-lg relative overflow-hidden border border-transparent hover:border-brand-accent hover:shadow-2xl transition-all duration-300">
                                <div class="relative z-10">
                                    <h3 class="text-3xl font-bold text-slate-800 mb-3">${produk.nama}</h3>
                                    <p class="text-slate-600 mb-6 text-lg leading-relaxed">${produk.deskripsi}</p>
                                    <div class="flex items-center justify-between mb-8">
                                        <span class="text-4xl font-bold text-brand-accent">Rp ${parseInt(produk.harga).toLocaleString('id-ID')}</span>
                                        <span class="bg-brand-primary text-white text-xs font-bold px-3 py-1 rounded-md">STOK TERSEDIA</span>
                                    </div>
                                    <a href="https://wa.me/${produk.wa}?text=Halo,%20saya%20tertarik%20dengan%20${encodeURIComponent(produk.nama)}" target="_blank" class="btn-whatsapp w-full text-center font-semibold py-3 px-8 rounded-full text-lg inline-flex items-center justify-center text-white">
